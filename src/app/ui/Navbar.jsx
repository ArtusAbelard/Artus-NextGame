...
export default function Navbar() {
    ...
    const [state, setstate] = useState(true) // func to change logo on click
    const panier = useSelector((state) => state.panier)
   
    
    return (
        <div className={`navbar z-50 h-[8rem] navshadow bg-[#0E0E0E] absolute  ${color.actual==true?"invert ":""}`}>  {/* function for darkmode*/}
            <div className="navbar-start min-[1440px]:ms-5 ">
                ...
                <ul tabIndex={0} className="menu menu-sm text-white dropdown-content mt-3 z-[1] p-2 shadow bg-[#0E0E0E] border-[1px] rounded-box w-52">
                    ...
                    <li className='hover:text-[#E08821]'><Link href='/produits'>Games</Link></li>  {/* burger nav link */}
            ...
            <div className="navbar-center hidden lg:flex">
                <ul className="text-xl menu menu-horizontal text-[#ECE6CE] px-1">
                    ...
                    <li><Link href='/produits'><button><span className={`box tracking-widest ${color.actual==true?"text-white":"invert"}`}>Games</span></button></Link></li>  {/* nav link */}
            ...
            <div className="navbar-end min-[1440px]:me-5">        
                <div className="dropdown dropdown-end ">  {/* hide or sho log in btn deppend of connexion status */}
                   {
                ...
                <div className="flex-none">
                    ...
                    <div tabIndex={0} role="button" className="btn btn-ghost btn-circle">
                        ...
                        <Image className='invert h-[1.5rem] w-[1.5rem] mt-1  ease-in duration-100 hover:w-[2rem] hover:h-[2rem] ' alt=""  src={star}></Image>  {/* drop down*/}
                        <span className="badge badge-sm indicator-item ">{panier.panier.length}</span>
                        </div>
                    </div>
                    <div tabIndex={0} className={`mt-3 z-50 border-[1px] card card-compact dropdown-content w-52 bg-[#0E0E0E] shadow  ${!connexion.status?"":"hidden"}`}>   {/* hide if connexion is false  */}
                        <div className="card-body">
                        <span className="font-bold text-base text-white ">You need to be logged for that !</span>
                        </div>
                    </div>
                    <div tabIndex={0} className={`mt-3  z-[1] border-[1px] card card-compact dropdown-content w-52 bg-[#0E0E0E] shadow ${!connexion.status?"hidden":""}`}>  {/* show if connexion is false  */}
                        <div className="card-body">
                        <span className="font-bold text-lg text-white">{panier.panier.length} Items</span>
                        {/* <span className=" text-[#E08821]">Subtotal: $999</span> */}
                        <p className='text-white'>Last Tow Items :</p>
                        <p>{panier.panier[panier.panier.length-1]}</p>
                        <p>{panier.panier[panier.panier.length-2]}</p>
