    ...
    return (
        <div className=" card card-compact w-96  bg-[#0E0E0E] shadow-xl mb-5">
                        ...
                        <div className="card-body h-[12rem] text-white">
                            ...
                            {panier.panier.includes(Element.title)?"":<Image onClick={()=>{panier.panier.includes(Element.title)?"":dispatch(panierset(Element.title),dispatch(imgset(Element.thumbnail)),sethide("hidden"))}} className={`invert cursor-pointer ms-1 w-[20px] h-[20px] hover:w-[25px] hover:h-[25px] transition-[3s] ${hide}`}  alt="" src={star}></Image>}
