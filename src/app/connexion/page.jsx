...
export default function Page() {
    ...
    function conect() {
      for (let i = 0; i < connexion.name.length; i++) {
        if (connexion.name[i]==name && connexion.password[i]==mdp) { //loging check if all match 
          setTimeout(() => {  //add timout to merge in home page
            router.push('/')  // merge in home page
          }, 1300);
          dispatch(logintru(!connexion.status)) // add conected status on connexion slice
        }
      }
    }
    return(
        <div className="min-w-screen  min-h-screen flex  items-center flex-col ">
            <Navbar></Navbar> 
            <div className={`w-screen min-h-[895px] mt-[6rem] flex items-center justify-center bg-[#1e1e1e] bglog max-[426px]:min-h-[750px] ${color.actual==true?"invert":""}`}> {/*darkmode func*/}
            <div className={`w-[30rem]  h-[30rem] rounded-xl bg-[#0E0E0E] flex flex-col items-center justify-center gap-5 bglogin boxshadowlog max-[426px]:w-[22rem] max-[426px]:h-[30rem] ${connexion.status==true?"hidden":""}`}>  {/* hide when login is true  */}
                ...
                <button onClick={()=>{dispatch(conect())}} className="btn text-xl bg-[#E08821] text-black hover:text-white">Connect</button> {/*call connect func*/}
            </div>
